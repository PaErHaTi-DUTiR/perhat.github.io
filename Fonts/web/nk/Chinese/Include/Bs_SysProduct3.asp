...
%>
	<TR>
		...
		<TD class='ML_Tdbg'><a href='Bs_Product.asp?BigClassName=<%=rsBigClass("BigClassName")%>'><%=rsBigClass("BigClassName")%></a></TD>
	</TR>
	<TR>
...
sub ArticleContent(intTitleLen)
	...
	do while not rsArticle.eof
		lb=""
			lb=lb & "<table width=100% border=0 cellspacing=3 cellpadding=0>"
			lb=lb & "<tr>"
			lb=lb & "<td width=30% rowspan=6>"
			lb=lb & "<div align=center><a href='Bs_ProductShow.asp?ArticleID=" & rsArticle("articleid") & "'>" 
			lb=lb & "<img border=0 src='"& rsArticle("DefaultPicUrl") &"' width=120 height=120>" 
			lb=lb & "</a></div></td>"
			lb=lb & "<td width=12% height=12>产品名称：</td>"
			lb=lb & "<td><a href='Bs_ProductShow.asp?ArticleID="& rsArticle("articleid") &"'>"& rsArticle("Title") &"</a></td>"
			lb=lb & "</tr>"

			lb=lb & "<tr>"
			lb=lb & "<td width=12% height=12>产品规格：</td>"
			lb=lb & "<td>"& rsArticle("Spec") &"</td>"
			lb=lb & "</tr>"

			lb=lb & "<tr>"
			lb=lb & "<td width=12% height=12>尺　　寸：</td>"
			lb=lb & "<td>"& rsArticle("Size") &"</td>"
			lb=lb & "</tr>"

			lb=lb & "<tr>"
			lb=lb & "<td height=12>产品类别：</td>"
			lb=lb & "<td><a href='Bs_Product.asp?BigClassName="& rsArticle("BigClassName") &"'>"& rsArticle("BigClassName") &"</a> → "
			lb=lb & "<a href='Bs_Product.asp?BigClassName="& rsArticle("BigClassName") &"&SmallClassName="& rsArticle("SmallClassName") &"'>"& rsArticle("SmallClassName") &""
			lb=lb & "</a></td>"
			lb=lb & "</tr>" 

'			lb=lb & "<tr>"
'			lb=lb & "<td height=18>产品编号：</td>"
'			lb=lb & "<td>"& rsArticle("Product_Id") &"</td>"
'			lb=lb & "</tr>"

			lb=lb & "<tr>"
			lb=lb & "<td height=12>产品信息：</td>"
			lb=lb & "<td><a href='Bs_ProductShow.asp?ArticleID="& rsArticle("articleid") &"'><img src='../Img/arrow_7.gif'border=0></a></td>"
			lb=lb & "</tr>"
			...
			lb=lb & "<tr>" 
			lb=lb & "<td width=50% height=12>"
			lb=lb & "<div align=center></div></td>"
			lb=lb & "<td width=50% height=12>"
			lb=lb & "<div align=center><a href='javascript:eshop("& rsArticle("Product_Id") &")'><img border=0 src='../img/addtocart.gif' width=95 height=17>"
			lb=lb & "</a></div></td>"
			lb=lb & "</tr>"
			lb=lb & "</table>"
			lb=lb & "</td>"
			...
			lb=lb & "<tr>" 
			lb=lb & "<td height=1 colspan=3 background='../img/kropki_poziom_naBialym.gif'><IMG height=1 src='../img/1x1_pix.gif' width=1></td>"
			lb=lb & "</tr>"
			lb=lb & "</table>"
		response.write lb
		rsArticle.movenext
...
sub ShowSearchTerm()
	...
	if keyword<>"" then
	  select case strField
		case "Title"
			response.Write "名称中含有 <font color=#851514>"&keyword&"</font> 的产品"
		case "Content"
			response.Write "说明含有 <font color=#851514>"&keyword&"</font> 的产品"
		case else
			response.Write "名称中含有 <font color=#851514>"&keyword&"</font> 的产品"
	  end select
	else
...
sub ShowArticleContent()
	...
	if ShowContentByPage="No" or ContentLen<=200000 then
		...
		if CurrentPage>1 then
			response.write "<a href='Bs_ProductShow.asp?ArticleID=" & ArticleID & "&ArticlePage=" & CurrentPage-1 & "'>上一页</a>&nbsp;&nbsp;"
		end if
		for i=1 to pages
		...
		next
		if CurrentPage<pages then
			response.write "&nbsp;<a href='Bs_ProductShow.asp?ArticleID=" & ArticleID & "&ArticlePage=" & CurrentPage+1 & "'>下一页</a>"
		end if
		response.write "</b></p>"
	end if
