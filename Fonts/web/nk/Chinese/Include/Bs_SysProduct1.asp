...
<TABLE cellSpacing=0  align="center" cellPadding=0 width="85%" border=0>
<%
dim i
...
%>
	<TR>
		...
		<TD class='ML_Tdbg'><a href='Bs_Product.asp?BigClassName=<%=rsBigClass("BigClassName")%>'language=javascript onmouseover="opencat(cat10<%=i%>000,&#13;&#10; img10<%=i%>000);"><%=rsBigClass("BigClassName")%></a></TD>
	</TR>
	<TR>
...
sub ShowArticleTotal()
	...
	if rsTotal.eof and rsTotal.bof then
		...
		totalPut=rsTotal(0)
		response.Write "共有 "& totalPut - 1 &" 个产品"
	end if
	rsTotal.close
	set rsTotal=nothing
end sub
...
sub ArticleContent(intTitleLen)
	...
	do while not rsArticle.eof
		...
		lb=""
			...
			lb=lb & "<img border=0 src='"& rsArticle("DefaultPicUrl") &"' width=130 height=90 class=bk></a>" 
			lb=lb & "</div></td>"
			lb=lb & "</tr>"
			...
			lb=lb & "<a href='Bs_ProductShow.asp?ArticleID="& rsArticle("articleid") &"'>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;"
