	...
	if rsBigClass_Down.bof and rsBigClass_Down.eof then 
		...
		do while not rsBigClass_Down.eof
			...
			rsClass.open sqlClass,conn,1,1
				...
				strTree= strTree & "<div align=center><img src=Img/arrow_6.gif width=13 height=11></div></td>"
				strTree= strTree & "<td width=150>"
				strTree= strTree & "<a href='Bs_Download.asp?BigClassName=" & rsBigClass_Down("BigClassName") & "'>"
	...
	if rsVote.bof and rsVote.eof then 
		...
		if rsVote("VoteType")="Single" then
			for i=1 to 8
				...
				response.Write "<input class='form'type='radio'name='VoteOption'value='" & i & "'>" & rsVote("Select" & i) & "<br>" 
			next
		else
			for i=1 to 8
				if trim(rsVote("Select" & i) & "")="" then exit for
				response.Write "<input class='form'type='checkbox'name='VoteOption'value='" & i & "'>" & rsVote("Select" & i) & "<br>"
			next
		end if
		response.write "<br><input class='form'name='VoteType'type='hidden'value='" & rsVote("VoteType") & "'>"
		response.write "<input class='form'name='Action'type='hidden'value='Vote'>"
		response.write "<input class='form'name='ID'type='hidden'value='" & rsVote("ID") & "'>"
...
sub ShowClass_DownGuide()
	response.write  "&nbsp;<a href='Bs_Download.asp'>下载</a>&nbsp;&gt;&gt;"
	if BigClassName="" and SmallClassName="" then
		response.write "&nbsp;所有下载"
	else
		if BigClassName<>"" then
			response.write "&nbsp;<a href='Bs_Download.asp?BigClassName=" & BigClassName & "'>" & BigClassName & "</a>&nbsp;&gt;&gt;&nbsp;"
			if SmallClassName<>"" then
				response.write "<a href='Bs_Download.asp?BigClassName=" & BigClassName & "&SmallClassName=" & SmallClassName & "'>" & SmallClassName & "</a>"
			else
				response.write "所有小类"
			end if
		end if
...
sub DownContent(intTitleLen)
	...
	do while not rsDown.eof
		...
		strTemp= strTemp & "<table width=100% border=0 cellspacing=3 cellpadding=0>"
                strTemp= strTemp & "<tr>"
                strTemp= strTemp & "<td width=20% rowspan=7>"
                ...
                strTemp= strTemp & "<td><a href=Bs_Download.asp?BigClassName=" & rsDown("BigClassName") & ">" & rsDown("BigClassName") & "</a> → "
                strTemp= strTemp & "<a href=Bs_Download.asp?BigClassName=" & rsDown("BigClassName") & "&SmallClassName=" & rsDown("SmallClassName") & ">" & rsDown("SmallClassName") & ""
                strTemp= strTemp & "</a></td>"
                strTemp= strTemp & "</tr><tr>" 
				
				strTemp= strTemp & "<td height=12>下载说明：</td>"
                strTemp= strTemp & "<td>"
                strTemp= strTemp & "<a href=bs_DownloadShow.asp?ID=" & rsDown("id") & "><img src=../Img/arrow_7.gif border=0></a></td>"
                strTemp= strTemp & "</tr><tr>"			
