...
%>
	<TR>
		...
		<TD width="262"><a href='En_Product.asp?EnBigClassName=<%=rsBigClass("EnBigClassName")%>'><%=rsBigClass("EnBigClassName")%></a></TD>
	</TR>
	<TR>
		<TD id=cat10<%=i%>000 <%if rsBigClass("EnBigClassName")=EnBigClassName then 
...
sub ArticleContent(intTitleLen)
	...
	do while not rsArticle.eof
		...
		lb=""
			...
			lb=lb & "<img border=0 src='"& rsArticle("DefaultPicUrl") &"' width=100 height=100 class=bk></a>" 
			lb=lb & "</div></td>"
			lb=lb & "</tr>"
...
sub ShowSearchTerm()
	...
	if keyword<>"" then
	  ...
	  response.write "&nbsp;All products" '所有产品
	end if
end sub
...
sub SearchResultTotal()
	...
	if rsTotal.eof and rsTotal.bof then
		totalPut=0
		response.write "Altogether 0 a product" '共有 0 个产品
	else
		totalPut=rsTotal(0)
		response.Write "Find altogether " & totalPut & " a product" '共找到  个产品
	end if
end sub
...
sub ShowSearchResult()
	...
	rsSearch.open sqlSearch,conn,1,1
 	if rsSearch.eof and rsSearch.bof then 
       		response.write "<p align='center'><br><br>Have not had or found any products</p>" '没有或没有找到任何产品
   	else 
   		if currentPage=1 then 
...
sub ShowAllClass()
	if rsBigClass.bof and rsBigClass.eof then 
		response.Write "&nbsp;There is not any column" 'There is not any column没有任何栏目
	else
